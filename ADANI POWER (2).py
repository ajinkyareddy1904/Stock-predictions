...
file_path = 'D:\STOCKS DATA\ADANIPOWER.csv' 
...
# Assuming the file has columns 'Date' and 'Close' for closing prices
file_path = 'D:\STOCKS DATA\ADANIPOWER1.csv' 
data = pd.read_csv(file_path)
...
if 'Close' not in data.columns:
    print("Error: 'Close' column not found in the CSV file.")
    exit()
data.rename(columns=lambda x: x.strip(), inplace=True)  # Remove leading/trailing whitespace
if 'close' in data.columns:
    data.rename(columns={'close': 'Close'}, inplace=True)
